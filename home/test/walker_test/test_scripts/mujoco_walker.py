...
parser = argparse.ArgumentParser(description="Run Mujoco walker Simulation")
...
def time_model(mj_model, steps):
    ...
    print("There are",mj_model.nu, "actuators in the walker")  # prints the number of actuators

    ctrl_signal = np.array([0, 0, 0, -1.0, 0, 0.5])
    ...
    with multiprocessing.Pool(processes=num_cores) as pool:
        ...
        pool.starmap(simulate_step, [(mj_model, mj_data_instances[_], num_cpu_steps, ctrl_signal) for _ in range(num_cores)])
...
def main():
    ...
    mj_model = mujoco.MjModel.from_xml_path("../xml/walker/walker.xml")
