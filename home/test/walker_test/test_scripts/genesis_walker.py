...
def simulate(scene, total_steps, walker):
    print()
    print("Warmup")
    ...
    position = walker.get_dofs_position()
    ...
    for _ in range(num_batch_steps):
        walker.control_dofs_position(position + torch.rand((n_envs, 9), device='cuda')*0.02, motor_dofs)
        scene.step()
...
def main():
    ...
    franka = scene.add_entity(
        gs.morphs.MJCF(file='../xml/walker/walker.xml'),
    )
