...
parser = argparse.ArgumentParser(description="Run MJX walker Simulation")
...
def time_model(mj_model, steps):
    ...
    curr_step = 0
    while True:
        mjx_data = mjx_data.replace(ctrl = ref_pos + jax.random.uniform(rng, shape=(n_envs, 6)) * 0.4 - 0.2)
        mjx_data = jit_step_n(mjx_model, mjx_data)
