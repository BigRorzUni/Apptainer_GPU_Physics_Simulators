...
print_help() {
    echo "USAGE: ./walker_test [ENGINE_TO_TEST]"
    echo ""
    echo "PARAMETERS:"
    ...
    echo ""
    echo "USAGE: ./walker_test SPECIAL_MODE [OUTPUT_IMAGE]"
    echo "SPECIAL MODES:"
...
if [[ "$#" -eq 0 ]]; then
    ...
    run_batch_sim test_scripts/newton_walker.py "N"

    echo "Running test on Genesis"
    run_batch_sim test_scripts/genesis_walker.py

    echo "Running test on Mujoco"
    python test_scripts/mujoco_walker.py $INPUT_LB $INPUT_UB $INPUT_POINTS

    echo "Running test on MJX"
    run_batch_sim "test_scripts/mjx_walker.py"
...
case "$1" in
    ...
    PLOT|plot)
        ...
        python plot_timings.py $SIMULATORS_ARG --base_dir "../walker_test/data" --file_type "$FILE_TYPE" $BATCH_SIZE
        ;;
